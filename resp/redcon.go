...
type Conn interface {
	...
	SetSession(session *client.Session)
...
func (c *conn) GetSession() *client.Session {
	return c.session
}

func (c *conn) SetSession(s *client.Session) {
	c.session = s
}
		...
		default:
			...
			for i := 0; i < len(b); i++ {
				...
				outer:
					...
					if len(cmd.Args) > 0 {
						// convert this to node command syntax
						var wr Writer
